...
def handle_ollama_chat(payload: dict) -> dict:
    """
    ...
    Output:  examples/output/py_ollama_<uid>.txt
    ...
    try:
        resp = httpx.post(f"{OLLAMA_BASE}/api/chat", json=request_body, timeout=120)
        resp.raise_for_status()
    except httpx.ConnectError as exc:
        raise RuntimeError(
            f"Ollama not available at {OLLAMA_BASE}. Start it with: ollama serve"
        ) from exc
    ...
    out_path = OUTPUT_DIR / f"py_ollama_{uuid.uuid4().hex[:8]}.txt"
